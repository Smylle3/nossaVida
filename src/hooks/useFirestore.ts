...
export default function useFirestore(collectionName: string) {
	...
	useEffect(() => {
		...
		const getData = async () => {
			try {
				const q = query(collection(db, collectionName), orderBy("uploadAt", "desc"));
	...
	const deleteImage = async (docName: string, pathName: string, imageName: string, imageFormat: string) => {
	...
	return {
		docs, isLoading, deleteImage, updateImage
	}
}
